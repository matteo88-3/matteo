...
export async function GET() {
  //console.log('[GET] fetch.jobcareers hit');
...
export async function POST(request: Request) {
  //console.log('--- [START] fetch.jobcareers API ---');
    ...
    */
    ...
    if (!API_URL || !API_KEY) {
      ...
      return NextResponse.json(
        { 
          success: false, 
          message: 'Server misconfiguration (missing API env)',
          jobcareerdata: [] 
        },
        { status: 500 }
      );
    ...
    //  External API call
    const response = await fetchWithTimeout(`${API_URL}/v2/jobs/fetch.jobs`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        success: true,
        message: data.message || 'Career jobs fetched successfully',
        jobcareerdata: data.jobcareerdata || []
      },
      { status: 200 }
    );
